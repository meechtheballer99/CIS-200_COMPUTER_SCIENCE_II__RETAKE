...
#include "sortedlist.cpp" //I had to add the inclusion of .cpp file of the ulist class to avoid the link error 
					//I was getting due to my classes implementing template classes; nonetheless I wanted my code to be easier to read so I still separated the sortedlist class
...
void ReadFromFile(ifstream& InputStream, SortedList<ItemType>& list); //use this function to read from the file
...
int main()
{
    cout << "---WELCOME: this program tests the implementation of a SORTED linked list  ----By Demetrius Johnson\n\n\n";
...
template<typename ItemType>
void ReadFromFile(ifstream& InputStream, SortedList<ItemType>& list) {
