...
void SortedList_DynArr<ItemType>::Insert(ItemType item){
	...
	if (!IsThere(item) && (length < 30)) { //insert between case
		...
		for(int i = length - 1; i >= 0; i--){ //begin i at last element in list
		
			if (top[i] < item) {

				top[i + 1] = top[i]; //set next element (which is a freed element) equal current element to shift it down the list

			}
			
			else { // location found for item insertion at an in between location
				top[i + 1] = item;
				length++;
				return;
			...
			if (i == 0) { //check: handle case where item must be inserted at top of list
